...
Stream.prototype.takeUntil = function (promise) {
  ...
  // isFuture
  Stream.Future(_utils.raceLP([promise.then(function (_) {
    return function () {
      return Stream.Empty;
    };
...
Stream.prototype.skipUntil = function (promise) {
  ...
  // isFuture
  Stream.Future(_utils.raceLP([this.promise.then(function (s) {
    return function () {
...
Stream.prototype.chunkBy = function (zero, f) {
  ...
  function go(acc, me) {
    ...
    if (me.isEmpty || me.isAbort) return zero === acc ? this : Stream.Cons(acc, me);
...
Stream.prototype.merge = function (s2) {
  ...
  return this.isEmpty ? s2 : this.isAbort ? this : this.isCons ? Stream.Cons(this.head, this.tail.merge(s2)) : !s2.isFuture ? s2.merge(s1) : Stream.Future(_utils.raceLP([this.promise.then(function (s) {
    return function () {
      return s.merge(s2);
    };
