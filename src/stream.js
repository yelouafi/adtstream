  ...
  this.isCons ?
    ( seed === undef ? 
        ...
        Promise.resolve(seed)
          .then(acc => this.tail.reduce( f, f(acc, this.head) ))
    ) :
   
  
  // isFuture
    this.promise.then( s => s.reduce(f, seed) );
};

// reduceRight : ( Stream a, (a,b) -> b | Promise b, b | Promise b ) => Promise b
Stream.prototype.reduceRight = function(f, seed = undef) {
  ...
  this.isCons ?
    ( seed === undef ? 
        this.tail.reduceRight(f, this.head) :
        Promise.resolve(seed)
          .then(acc => this.tail.reduceRight( f, acc))
          .then(acc2 => f(this.head, acc2))
    ) :
   
  
  // isFuture
    this.promise.then( s => s.reduceRight(f, seed) );
};
...
Stream.prototype.window = function(size, min = 0) { 
  return this.scan( (p,c) => p.length < size ? [...p,c] : [...p.slice(1), c], [] )
...
Stream.prototype.combineWith = function(s2, f, latest1=undef, latest2=undef) {
  
  return  this.isEmpty ? 
    ( latest1 !== undef ? s2.map(x => f(latest1, x)) : this ) :
  
  this.isAbort ? this :
  
  this.isCons ?
    ( latest2 !== undef ?
        Stream.Cons( f(this.head, latest2), this.tail.combineWith(s2, f, this.head, latest2) ) :
        this.tail.combineWith(s2, f, this.head, latest2)
    ) :
  
  /* this.isFuture */
  ( !s2.isFuture ?
      s2.combineWith(this, flip(f), latest2, latest1) :
      Stream.Future(
        raceL([
          this.promise.then( s => () => s.combineWith(s2, f, latest1, latest2), Stream.Abort ),
          s2.promise.then( s => () => s2.combineWith(this, flip(f), latest2, latest1), Stream.Abort )  
        ])
      )
  )
...
Stream.prototype.combine = function(s2) {
  return this.combineWith(s2, (x,y) => [x,y]);
}

// Stream.combine : [Stream a] -> Stream [a]
Stream.combine = function(...args) {
  return args.reduce((acc, s, i) => 
    i === 1 ?
      acc.combine(s) :
      acc.combineWith(s, (arr, x) => [...arr, x])
  );
}
...
Stream.prototype.zipWith = function(s2, f) { 
  ...
  this.isCons && s2.isCons ?
    Stream.Cons( f(this.head, s2.head), this.tail.zipWith(s2.tail, f) ) :
    
  this.isCons && s2.isFuture ?
    Stream.Future(
      s2.promise.then( s => this.zipWith(s, f), Stream.Abort  )
    ) :
    
  // this.isFuture && (s2.isCons || s2.isFuture)
  Stream.Future(
    this.promise.then( s => s.zipWith(s2, f), Stream.Abort )  
  );
};

// zip : (Stream a, Stream b) -> Stream [a,b]
Stream.prototype.zip = function(s2) {
  return this.zipWith(s2, (x,y) => [x,y]);
};
